...
/* ---- Payment endpoints (Dynamic) ---- */
$pay = [];

// UPI
if (!empty($_ENV['PAYMENT_UPI_ID'])) {
    $pay[] = ['name'=>'UPI ID', 'addr'=>$_ENV['PAYMENT_UPI_ID']];
}

// Binance
if (!empty($_ENV['PAYMENT_BINANCE_ID'])) {
    $pay[] = ['name'=>'BINANCE ID / PAY', 'addr'=>$_ENV['PAYMENT_BINANCE_ID']];
}

// Crypto
if (!empty($_ENV['PAYMENT_BTC_ADDR'])) {
    $pay[] = ['name'=>'BTC (Bitcoin)', 'addr'=>$_ENV['PAYMENT_BTC_ADDR'], 'ticker'=>'btc'];
}
if (!empty($_ENV['PAYMENT_LTC_ADDR'])) {
    $pay[] = ['name'=>'LTC (Litecoin)', 'addr'=>$_ENV['PAYMENT_LTC_ADDR'], 'ticker'=>'ltc'];
}
if (!empty($_ENV['PAYMENT_TRX_ADDR'])) {
    $pay[] = ['name'=>'TRX (Tron)', 'addr'=>$_ENV['PAYMENT_TRX_ADDR'], 'ticker'=>'trx'];
}
if (!empty($_ENV['PAYMENT_USDT_TRC20'])) {
    $pay[] = ['name'=>'USDT (TRC20)', 'addr'=>$_ENV['PAYMENT_USDT_TRC20'], 'fixed_usdt'=>true];
}
if (!empty($_ENV['PAYMENT_USDT_BEP20'])) {
    $pay[] = ['name'=>'USDT (BEP20)', 'addr'=>$_ENV['PAYMENT_USDT_BEP20'], 'fixed_usdt'=>true];
}

if (empty($pay)) {
    $pay[] = ['name'=>'No methods available', 'addr'=>'Please contact admin'];
}

$qrLink = $_ENV['PAYMENT_QR_LINK'] ?? '';
?>
<div class="space-y-6">
  ...
  <div id="step2" class="hidden space-y-4">
    ...
    <div class="rounded-2xl border border-white/10 bg-white/5 p-5">
      ...
      <?php if (!empty($qrLink)): ?>
      <div class="mb-4 flex flex-col items-center justify-center p-4 rounded-xl border border-white/10 bg-white/5">
        <div class="text-xs text-slate-400 mb-2 font-bold uppercase tracking-wider">Scan to Pay</div>
        <img src="<?=htmlspecialchars($qrLink,ENT_QUOTES)?>" alt="Payment QR" class="rounded-lg shadow-lg max-w-[200px] border border-white/20">
      </div>
      <?php endif; ?>
